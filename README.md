...
[`gpx-events.json`](gpx-events.json) — so an unchanged working tree after a run means the event data has not moved, and
that is how the workflow decides whether there is anything to commit instead of pushing fresh timestamps four times a
day. Note that GitHub disables a scheduled workflow after 60 days without a commit to the repository; re-enable it from
the Actions tab if the feeds ever go stale.
